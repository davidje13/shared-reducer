...
describe('ReadWriteStruct', () => {
  ...
  it('allows data changes', () => {
    ...
    expect(() => permission.validateWrite(newValue, oldValue)).not(toThrow());
  });

  it('rejects adding read-only fields', () => {
    const oldValue = { foo: 'a' };
    const newValue = { foo: 'a', blocked: 'b' };
    expect(() => permission.validateWrite(newValue, oldValue)).toThrow('Cannot add field blocked');
  });

  it('rejects removing read-only fields', () => {
    const oldValue = { blocked: 'a' };
    const newValue = {};
    expect(() => permission.validateWrite(newValue, oldValue)).toThrow(
      'Cannot remove field blocked',
    );
  });
  ...
  it('rejects editing read-only fields', () => {
    ...
    expect(() => permission.validateWrite(newValue, oldValue)).toThrow('Cannot edit field blocked');
  });
